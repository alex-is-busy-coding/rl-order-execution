...
def train_agent(env: OrderExecutionEnv, agent: DQNAgent) -> List[float]:
    ...
    writer = SummaryWriter(log_dir=f"runs/dqn_episodes_{rl_conf.episodes}_lr_{rl_conf.lr}")
    ...
    for episode in range(rl_conf.episodes):
        ...
        if (episode + 1) % 50 == 0:
            ...
            logger.info(f"Episode {episode+1:03d} | Avg Reward: {avg_rew:.2f} | Epsilon: {agent.epsilon:.2f}")
