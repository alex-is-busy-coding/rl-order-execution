...
def evaluate_agent(env: OrderExecutionEnv, agent: DQNAgent, num_trials: int = 100):
    ...
    twap_idx = min(range(len(env.action_multipliers)), 
...
def plot_distribution(df: pd.DataFrame, filename: str):
    ...
    plt.axvline(mean_savings, color='green', linestyle='-', label=f"Mean (${mean_savings:.2f})")
    ...
    plt.xlabel("Cost Savings ($) - Positive = Outperformance, Negative = Underperformance")
...
def visualize_trajectory(env: OrderExecutionEnv, agent: DQNAgent, filename: str = 'output/execution_analysis.png'):
    """
    ...
    twap_inventory = [
        max(0, sim_conf.total_shares - (i * (sim_conf.total_shares / sim_conf.time_horizon)))
    ...
    ax1.plot(twap_inventory, label='TWAP (Benchmark)', linestyle='--', color='orange', linewidth=2)
